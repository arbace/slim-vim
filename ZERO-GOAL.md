...
**This document is iterative, and so far it has ten phases.** Phase 0 is the seed,
...
phase 5 leaves the command line as `+{command}` and `-T {term}`, and phases 6 to 9
are *no filesystem* on request: the editor loses every way to write a file, then
every way to read one, then every way to name another one to edit, and finally the
machinery that read the bytes — which by then nothing could reach. Phases
