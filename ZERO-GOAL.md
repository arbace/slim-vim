...
**This document is iterative, and so far it has six phases.** Phase 0 is the seed,
...
and phase 5 leaves the command line as `+{command}` and `-T {term}`. Phases are added one at a time, each on the user's own
