...
**This document is iterative, and so far it has nine phases.** Phase 0 is the seed,
...
phase 5 leaves the command line as `+{command}` and `-T {term}`, and phases 6, 7 and
8 are *no filesystem* on request: the editor loses every way to write a file, then
every way to read one, and then every way to name another one to edit. Phases
