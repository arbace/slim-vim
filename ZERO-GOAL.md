...
| | input | after |
| --- | --- | --- |
| lines | 79,865 | **80,421** (+556) |
| functions | 1,719 | **1,736** (+17) |
...
absolute numbers move when phase 14's nineteen are restyled into the shape
`funcreach.py` can see, and the difference does not.
...
`tools/zerodelta.sh`. `make zero-tip` is 48 s. Its boundary is `542eb8af1b2c`.
...
`make zero-verify` reproduces all sixteen boundaries, r0 to r15, in **110 s** of wall
time over 816 s of phases, and all 107 whim and slim implementation keys are unchanged.
