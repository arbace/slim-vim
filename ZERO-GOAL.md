...
**This document is iterative, and so far it has seven phases.** Phase 0 is the seed,
...
phase 5 leaves the command line as `+{command}` and `-T {term}`, and phase 6 is the
first piece of *no filesystem*: the editor loses every way to write a file. Phases
