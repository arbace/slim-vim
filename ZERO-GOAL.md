...
**This document is iterative, and so far it has eight phases.** Phase 0 is the seed,
...
phase 5 leaves the command line as `+{command}` and `-T {term}`, and phases 6 and 7
are *no filesystem* on request: the editor loses every way to write a file, and
then every way to read one. Phases
