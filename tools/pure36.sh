#!/bin/sh
# Pure phase 36 -- the completion keys stop being keys.  See PURE-GOAL.md.
#
# Usage: tools/pure36.sh <work-dir>      (run from the repository root)
#
...
work=${1:?usage: pure36.sh <work-dir>}
f="$work/pure-vim.c"
...
if python3 tools/complcheck.py "$work/pure-vim"; then
...
if ! python3 tools/arrowcheck.py "$work/pure-vim"; then
