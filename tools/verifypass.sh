...
#
# Usage: tools/verifypass.sh slim|whim [phase...]     (run from the repository root)
...
if [ "${1:-}" = "--one" ]; then
    ...
    if [ "$n" = 0 ]; then in_tar=$PBUILD/input.tar; in_want=$(cat "$PBUILD/input.sha256")
    else in_tar=$PBUILD/$TAG$((n - 1)).tar; in_want=$(want_of $((n - 1))); fi
    want=$(want_of "$n")
    d=$scratch/$TAG$n
    ...
    [ -n "$want" ] || { echo "$TAG$n UNRECORDED" > "$res"; exit 0; }
    start=$(date +%s)
    (
        cd "$d"
        ...
        if [ "$(cat in.sha256)" != "$in_want" ]; then
            echo "$TAG$n INPUT $(cut -c1-12 in.sha256) is not the recorded $(printf %s "$in_want" | cut -c1-12)" > "$res"
            exit 0
        fi
        rm -f in.tar
        ...
        if [ "$(cat out.sha256)" = "$want" ]; then
            echo "$TAG$n ok $(cut -c1-12 out.sha256) ${secs}s" > "$res"
        else
            echo "$TAG$n DIFFERS got $(cut -c1-12 out.sha256), recorded $(printf %s "$want" | cut -c1-12) ${secs}s -- $d" > "$res"
        fi
    )
    exit 0
fi

pipe=${1:?usage: verifypass.sh slim|whim [phase...]}
shift
jobs=${JOBS:-$(nproc)}
. tools/pipeline.sh "$pipe"
if [ $# -gt 0 ]; then PHASE_LIST=$*; fi
scratch=$(mktemp -d "${TMPDIR:-/tmp}/verifypass-$PIPE.XXXXXX")
start=$(date +%s)
n=0; for _p in $PHASE_LIST; do n=$((n + 1)); done
printf '  %-12s %d phases of %s, %d at a time, in %s\n' "verifypass" "$n" "$PIPE" "$jobs" "$scratch"

printf '%s\n' $PHASE_LIST | xargs -P "$jobs" -I{} sh tools/verifypass.sh --one "$PIPE" {} "$scratch"
