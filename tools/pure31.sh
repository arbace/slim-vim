#!/bin/sh
# Pure phase 31 -- C indenting.  See PURE-GOAL.md.
#
...
# --local WITHOUT --strict, which phase 18 settled: the global-read guard runs
...
ai=$(cd "$work" && rm -rf .citest && mkdir .citest && cd .citest \
     && printf '    one\n' > f.txt \
     && ../pure-vim -e -s -c 'set autoindent' -c 'normal Gotwo' -c 'wq' f.txt \
            </dev/null >/dev/null 2>&1
     sed -n 2p f.txt | cat -A | head -1)
probe() {
    (cd "$work" && ./pure-vim -e -s -c "set $1" -c 'qa!' </dev/null >/dev/null 2>&1)
    echo $?
}
rm -rf "$work/.citest"
case $ai in
