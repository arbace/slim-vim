#!/bin/sh
# Pure phase 30 -- `[[=a=]]` stops meaning "a with any accent".  See PURE-GOAL.md.
#
...
t=$(cd "$work" && rm -rf .eqtest && mkdir .eqtest && cd .eqtest \
    && printf 'xax\nx\303\241x\nx=x\n' > f.txt \
    && ../pure-vim -e -s -c 's/[[=a=]]/#/g' -c 'wq' f.txt </dev/null >/dev/null 2>&1
    tr '\n' ' ' < f.txt)
alpha=$(cd "$work" && cd .eqtest && printf 'a1b\n' > g.txt \
    && ../pure-vim -e -s -c 's/[[:alpha:]]/#/g' -c 'wq' g.txt </dev/null >/dev/null 2>&1
    tr '\n' ' ' < g.txt)
rm -rf "$work/.eqtest"
case $t in
