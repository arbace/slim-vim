#!/bin/sh
# Pure phase 28 -- nothing in the file is unreachable.  See PURE-GOAL.md.
#
# Usage: tools/pure28.sh <work-dir>      (run from the repository root)
#
# THE LAST PHASE, and the only one that removes nothing in particular.  Every
# phase before it sweeps what its own cut orphaned; this one asks the whole file
# a question none of them can: IS ANYTHING LEFT THAT NOTHING REACHES?
#
# The sweep every phase runs covers four of the five kinds:
#
...
#
# TWO KINDS WERE COVERED BY NOTHING AT ALL, and both had already been met by
# hand in a phase tool because nothing would find them:
#
#   enumerators   gcc has no warning, typereach counts them only to decide
#                 whether their enum is alive.  82 of them.
#   struct fields A FIELD IS NOT A VARIABLE: no warning names one that nothing
#                 reads and the sweep cannot see it.  77 of them -- b0_uname
#                 and eight fields of memfile_T were two this pipeline had to
#                 name by hand, in phases that should not have had to care.
#
# WHAT THIS PHASE ASSERTS, after removing what it finds: all six counts are
# zero.  That is the invariant, and it is why this is a phase rather than a
# cleanup -- a phase runs every pass, so the invariant is checked every pass.
#
# THE ENUMERATOR TRAP: an enumerator's value is its position, so deleting one
# renumbers every implicit one after it, and several enums here index a
# parallel table.  The values are read from DWARF before and after -- the
# compiler has already done the arithmetic -- the first survivor after each
# deleted run is pinned to the value it had, and every surviving name must come
# back with the value it went in with.  That is stronger than the build, which
# compiles a silently renumbered table without complaint.
#
# THE DELTA: none.  Nothing removed here was reachable, so nothing that ran
# before can stop running.
set -eu

work=${1:?usage: pure28.sh <work-dir>}
f="$work/pure-vim.c"
...
# the first run of this phase came out of one pass with three enumerators still
# dead.  Same argument as the sweep's own loop, one level up -- and the values
# passed to deadenums stay the ORIGINAL ones every round, because it is the
# original numbering that has to survive.
round=0
while :; do
    round=$((round + 1))
